    ...
    <div class="modal-dialog modal-dialog-centered modal-xl modal-dialog-scrollable">
      <div class="modal-content">
        ...
        <div class="modal-body px-5">
          <VForm v-slot="{ errors }" id="OrderModal"
            class="row g-3" @submit="updateOrder(newOrder, newOrder.id)">
            ...
            <div class="col-4">
              <div class="input-group mb-3">
                ...
                <input type="text" class="form-control" aria-label="ID" aria-describedby="id"
                  disabled :value="newOrder.id"/>
              </div>
            </div>
            <div class="col-4">
              <span class="me-2">付款狀態: </span>
              <input type="checkbox" class="btn-check" id="btn-check-outlined"
                autocomplete="off" v-model="newOrder.is_paid"/>
              <label class="btn" for="btn-check-outlined"
                :class="{'btn-outline-success': newOrder.is_paid,
                ...
                <tbody v-if="newOrder.products">
                  <tr v-for="(product, key) in newOrder.products" :key="key">
                    <td>
                      ...
                      @click="deleteOrderItem(key)">X</button>
                    </td>
            ...
            <div class="col-4">
              ...
              <VField id="username" name="訂購人姓名" type="text"
                class="form-control" :class="{ 'is-invalid': errors['訂購人姓名'] }"
                placeholder="請輸入 訂購人姓名" rules="required" v-model="newOrder.user.name"></VField>
              <ErrorMessage name="訂購人姓名" class="invalid-feedback"></ErrorMessage>
            </div>
            <div class="col-4">
              <label for="email" class="form-label">訂購人Email:</label>
              <VField id="email" name="訂購人Email" type="email"
                class="form-control" :class="{ 'is-invalid': errors['訂購人Email'] }"
                placeholder="請輸入 訂購人Email" rules="required" v-model="newOrder.user.email">
                </VField>
              <ErrorMessage name="訂購人Email" class="invalid-feedback"></ErrorMessage>
            </div>
            <div class="col-4">
              <label for="tel" class="form-label">訂購人電話:</label>
              <VField id="tel" name="訂購人電話" type="tel"
                class="form-control" :class="{ 'is-invalid': errors['訂購人電話'] }"
                placeholder="請輸入 訂購人電話" rules="required" v-model="newOrder.user.tel"></VField>
              <ErrorMessage name="訂購人電話" class="invalid-feedback"></ErrorMessage>
            </div>
            <div class="col-6">
              <label for="address" class="form-label">訂購人地址:</label>
              <VField id="address" name="訂購人地址" type="address"
                class="form-control" :class="{ 'is-invalid': errors['訂購人地址'] }"
                placeholder="請輸入 訂購人地址" rules="required" v-model="newOrder.user.address"></VField>
              <ErrorMessage name="訂購人Email" class="invalid-feedback"></ErrorMessage>
            </div>
            <div class="col-12">
              <label for="message" class="form-label">留言備註</label>
              <textarea name="" id="message" cols="20" rows="5"
                v-model="newOrder.user.message" class="form-control"></textarea>
            </div>
          </VForm>
        </div>
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-theme text-white"
            :disabled="loadingItem === newOrder.id" form="OrderModal">確認</button>
        </div>
      </div>
    </div>
  </div>
